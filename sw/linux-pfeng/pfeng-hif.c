...
void pfe_hif_drv_set_idex_resend_cfg(pfe_hif_drv_t *hif_drv, const uint32_t idex_resend_count, const uint32_t idex_resend_delay)
{
	...
	hif_drv->idex_resend_delay = idex_resend_delay;
}
...
void pfe_hif_drv_get_idex_resend_cfg(const pfe_hif_drv_t *hif_drv, uint32_t *idex_resend_count, uint32_t *idex_resend_delay)
{
	...
	*idex_resend_delay = hif_drv->idex_resend_delay;
}
...
static int pfeng_hif_idex_create(struct pfeng_priv *priv, int idx)
{
	...
	pfe_hif_drv_set_idex_resend_cfg(&ihc_chnl->hif_drv, priv->idex_resend_count, priv->idex_resend_delay);
