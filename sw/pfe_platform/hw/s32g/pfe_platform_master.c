...
errno_t pfe_platform_init(const pfe_platform_config_t *config)
{
	...
	ret = pfe_feature_mgr_add_fw_modules(pfe.classifier, pfe.util);
    if (EOK != ret)
	{
		goto exit;
	}
	...
	/*	Deactivate class features*/
	if (TRUE == pfe_feature_mgr_is_available("hif_isolate_performance"))
	{
		if (EOK != pfe_feature_mgr_set_val("hif_isolate_performance", 0))
		{
			NXP_LOG_INFO("HIF isolate performance TMU feature enabled!\n");
		}
	}
